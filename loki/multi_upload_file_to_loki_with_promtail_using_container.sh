...
do
case $i in
    -le=*|--loki_endpoint=*) loki_endpoint="${i#*=}"; shift ;;
    -lh=*|--last_hour=*) last_hour="${i#*=}"; shift ;;
    -h|--help|*) show_usage ;;
esac
done
...
deploy () {
  ...
  for HOUR in  $(eval echo "{00..$last_hour}"); do
    ...
    DEPLOYMENT_NAME="uploadtoloki."$DAY"."$HOUR
    echo "starting container upload for $FILE_NAME_JSON"
    oc delete deployment --ignore-not-found=true --wait=true $DEPLOYMENT_NAME 
    ...
    if [ "$hold_until_completion" = "false" ] ; then
      ...
      while true; do
        sleep 10  
        UPLOADER_POD=$(oc get pod -l app=$DEPLOYMENT_NAME -o jsonpath="{.items[0].metadata.name}" 2>/dev/null )
